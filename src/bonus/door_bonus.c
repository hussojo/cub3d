...
int	check_closable(t_ray *ray)
{
	...
	if (ray->data->map[y][x] == 'O')
		ray->data->map[y][x] = 'D';
	else if (ray->data->map[y][(int)(x - 1.5)] == 'O')
		ray->data->map[y][(int)(x - 1.5)] = 'D';
	else if (ray->data->map[y][(int)(x + 1.5)] == 'O')
		ray->data->map[y][(int)(x + 1.5)] = 'D';
	else if (ray->data->map[(int)(y - 1.5)][x] == 'O')
		ray->data->map[(int)(y - 1.5)][x] = 'D';
	else if (ray->data->map[(int)(y + 1.5)][x] == 'O')
		ray->data->map[(int)(y + 1.5)][x] = 'D';
	else
		return (0);
...
int	check_openable(t_ray *ray)
{
	...
	if (ray->data->map[y][x] == 'D')
		ray->data->map[y][x] = 'O';
	else if (ray->data->map[y][(int)(x - 1.5)] == 'D')
		ray->data->map[y][(int)(x - 1.5)] = 'O';
	else if (ray->data->map[y][(int)(x + 1.5)] == 'D')
		ray->data->map[y][(int)(x + 1.5)] = 'O';
	else if (ray->data->map[(int)(y - 1.5)][x] == 'D')
		ray->data->map[(int)(y - 1.5)][x] = 'O';
	else if (ray->data->map[(int)(y + 1.5)][x] == 'D')
		ray->data->map[(int)(y + 1.5)][x] = 'O';
	else
		return (0);
